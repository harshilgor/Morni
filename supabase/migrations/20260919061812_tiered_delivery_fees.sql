-- Keep the database order authority in sync with checkout's tiered delivery fees.
-- The AED 100–200 band covers the previously unspecified AED 150–200 gap.
...
begin
  ...
  small_fee := 0;
  delivery_fee := case
    when subtotal >= 200 then 0
    when subtotal >= 100 then 3
    when subtotal >= 50 then 5
    else 7
  end;
