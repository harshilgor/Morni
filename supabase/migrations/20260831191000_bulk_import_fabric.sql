alter table public.bulk_import_items add column if not exists fabric text;
...
begin
  for item in select * from public.bulk_import_items where import_id = p_import_id and status in ('pending','failed') order by created_at loop
    begin
      ...
      insert into public.products(store_id,category_id,title,description,fabric,price_aed,stock,sizes,image_urls,is_available)
      select i.store_id,category_id,item.title,item.description,item.fabric,item.price_aed,item.stock,case when item.category_slug in ('gifting','hamper','hampers') then '{}'::text[] else item.sizes end,item.image_urls,true from public.bulk_imports i where i.id=p_import_id returning id into created_id;
