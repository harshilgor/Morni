-- Enrich the rider workspace without changing the existing delivery action RPCs.
-- The client can show history, support, and location freshness in one request.
...
begin
  ...
  return jsonb_build_object(
    ...
    'partner', jsonb_build_object(
      'name', v_partner.name,
      'support_email', v_partner.support_email
    ),
    'jobs', coalesce((
    ...
    'history', coalesce((
      select jsonb_agg(jsonb_build_object(
        'id', history_job.id,
        'status', history_job.status,
        'order_number', order_row.order_number,
        'store_name', store.name,
        'delivery_area', order_row.delivery_area,
        'delivered_at', history_job.delivered_at,
        'failed_at', history_job.failed_at,
        'failure_reason', history_job.failure_reason,
        'updated_at', history_job.updated_at
      ...
      join public.stores store on store.id = order_row.store_id
      where history_job.driver_id = v_driver.id
        and history_job.status in ('delivered', 'failed', 'cancelled')
