-- Include handoff state and proof count in the single rider workspace payload.
...
begin
  ...
  return jsonb_build_object(
    ...
    'history', coalesce((
      ...
      join public.stores store on store.id = order_row.store_id
      where history_job.driver_id = v_driver.id and history_job.status in ('delivered', 'failed', 'cancelled') and history_job.updated_at >= now() - interval '30 days'
    ), '[]'::jsonb)
  );
end;
$$;
