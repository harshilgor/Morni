...
export async function POST(request: Request) {
  ...
  if (importId) {
    ...
    if (!items.length) {
      const { data: pending } = await admin.from("bulk_import_items").select("title,product_tag,description,fabric,category_slug,price_aed,stock,sizes,size_stock,image_urls,variant_groups").eq("import_id", importId).eq("status", "failed");
      items = (pending ?? []).map((item) => ({ title: item.title, productTag: item.product_tag ?? "", description: item.description ?? "", fabric: item.fabric ?? null, categorySlug: item.category_slug, priceAed: Number(item.price_aed), stock: item.stock, sizes: item.sizes ?? [], sizeStock: item.size_stock ?? {}, images: item.image_urls ?? [], variants: item.variant_groups ?? [] }));
    }
  } else {
