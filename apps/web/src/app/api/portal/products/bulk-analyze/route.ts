...
export async function POST(request: Request) {
  ...
  const prompt = `You are matching product photos for a fashion marketplace. Compare EVERY uploaded image against every other uploaded image. Group by physical product identity. A different color, colorway, print, pattern, material, or design is a DIFFERENT product even when the garment type and silhouette are identical. Never group distinct colorways together merely because both are co-ord sets, dresses, kurtas, or another shared category. Different angles, poses, front/back views, close-ups, or models are the same product ONLY when colorway, print, material, and design details match. If color or identity is uncertain, keep photos separate and set needsReview true. Every group must have a specific visible colorName, or an empty string only when genuinely unclear. Use ONLY the stable photo labels supplied below. Every label must appear exactly once across all groups. Suggest conservative metadata only; do not invent brand, fabric, measurements, price, stock, or sizes. Categories: ${JSON.stringify(mergeBrowseCategories((categories ?? []) as BrowseCategory[]).map(({ name, slug }) => ({ name, slug })))}. The photo labels are: ${JSON.stringify(labelledImages.map(({ label, name }) => ({ label, name })))}.`;
