...
export function ProductCardImage({ src, alt, priority = false }: { src?: string; alt: string; priority?: boolean }) {
  ...
  return <Image src={src} alt={alt} fill sizes="(max-width: 640px) 48vw, (max-width: 1024px) 30vw, 280px" quality={100} priority={priority} onError={() => setFailedSrc(src)} className="object-cover transition duration-500 group-hover:scale-[1.04]" />;
}
