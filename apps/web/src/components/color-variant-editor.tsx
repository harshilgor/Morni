...
import { MEDIA_ACCEPT, MEDIA_ACCEPT_LABEL, validateImageFile } from "@/lib/media-upload";
...
export function ColorVariantEditor({
  ...
  function addImages(key: string, files: FileList | File[] | null) {
    ...
    setImageErrors((current) => ({ ...current, [key]: rejection }));
    if (accepted.length === 0) return;

    onChange(
      value.map((draft) =>
        draft.key === key
          ? { ...draft, images: [...draft.images, ...accepted] }
          : draft,
      ),
    );
  }
            ...
            >
              ...
              >
                ...
                {draft.images.length > 0 ? (
                  <div className="grid grid-cols-3 gap-2 sm:grid-cols-5">
                    {draft.images.map((image, imageIndex) => (
                      <div
                        key={image.id}
                        className="group relative overflow-hidden rounded-xl border border-line bg-[#f2f2f3]"
                      >
                        <button
                          type="button"
                          onClick={() =>
                        ...
                        <div className="absolute inset-x-1.5 bottom-1.5 flex gap-1 opacity-0 transition group-focus-within:opacity-100 group-hover:opacity-100">
                          {imageIndex !== 0 ? (
                            <button
                              type="button"
                              disabled={disabled}
                              onClick={() => makeMainImage(draft.key, image.id)}
                              className="flex-1 rounded-full bg-white/90 px-1.5 py-1 text-[10px] font-medium text-ink shadow-sm disabled:opacity-50"
                            >
                              Main
                            </button>
                          ) : null}
                          <button
                            ...
                            className="flex-1 rounded-full bg-ink/85 px-1.5 py-1 text-[10px] font-medium text-white disabled:opacity-50"
                          >
                            Remove
                          </button>
                        </div>
                ...
                <button
                  ...
                  className={`flex w-full flex-col items-center justify-center rounded-2xl border-2 border-dashed px-4 py-7 text-center transition disabled:opacity-50 ${
                    draggingKey === draft.key
                      ? "border-accent bg-[#fff0f4]"
                      : "border-[#d4d4d8] bg-[#f2f2f3] hover:border-accent/50 hover:bg-[#ebebed]"
                  }`}
                >
                  <span className="flex h-11 w-11 items-center justify-center rounded-full bg-white text-2xl leading-none text-accent-deep shadow-sm">
                    +
                  </span>
                  <span className="mt-2.5 text-sm font-medium text-ink">
                    {draft.images.length > 0
                      ? "Add more photos"
                      : "Upload photos for this color"}
                  </span>
                  <span className="mt-1 text-xs text-muted">
                    Drag and drop or click to select several photos at once
                  </span>
                ...
                <input
                  ref={(node) => {
                    fileInputs.current[draft.key] = node;
                  }}
                  type="file"
                  accept={MEDIA_ACCEPT}
                  multiple
                    ...
                    {PRODUCT_SIZES.map((size) => {
                      ...
                      return (
                        <button
                          ...
                          className={`min-w-10 rounded-lg border px-2.5 py-1.5 text-xs font-medium transition ${
                            selected
