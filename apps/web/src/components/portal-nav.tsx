  ...
  return (
    <div className="flex shrink-0 gap-1 lg:block lg:space-y-1">
      ...
      {links.map((link) => {
        ...
        return <Link key={link.href} href={link.href} className={`flex shrink-0 items-center gap-3 rounded-lg border px-3 py-2.5 text-sm font-semibold transition ${active ? "border-[#bfd0c8] bg-[#e3eee9] text-[#1f594f] shadow-[0_1px_2px_rgba(20,35,29,0.05)]" : "border-transparent text-[#52615b] hover:border-[#d2dad6] hover:bg-white hover:text-[#1f302a]"}`}><PortalIcon name={link.icon} className="h-[18px] w-[18px]" /><span>{link.label}</span></Link>;
      })}
    </div>
  );
}
    ...
    <aside className="z-40 border-b border-[#c6d0cb] bg-[#f8faf9] lg:sticky lg:top-0 lg:h-screen lg:w-[15.5rem] lg:shrink-0 lg:border-b-0 lg:border-r">
      <div className="flex h-full flex-col">
        ...
        <div className="mt-auto hidden border-t border-[#d5ddd9] p-3 lg:block">
          <Link href={storefrontHref} className="flex items-center gap-3 rounded-lg border border-[#bcc8c2] bg-white p-3 shadow-[0_1px_2px_rgba(20,35,29,0.05)] transition hover:border-[#8fa39a] hover:bg-[#f7faf8]"><span className="grid h-8 w-8 place-items-center rounded-lg bg-[#e8efec] text-[#315f54]"><PortalIcon name="store" /></span><span className="min-w-0 flex-1"><span className="block truncate text-xs font-semibold text-[#283832]">{store?.name ?? "Your storefront"}</span><span className="mt-0.5 block text-[11px] text-[#687770]">{store?.is_active ? "View shopper page" : "Complete store setup"}</span></span><PortalIcon name="external" className="h-3.5 w-3.5 text-[#687770]" /></Link>
