...
export function TrendingForYou({ products }: { products: (TrendingCandidate & { stores: { slug: string } })[] }) {
  ...
  return <ProductRail id="trending-for-you" title={`${label}!`} subtitle="Fresh picks selected from what is available today." products={rail} href={result.categorySlug ? `/categories/${result.categorySlug}` : "/search"} sharp />;
}
