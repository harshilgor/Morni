...
export function DeliveryInviteJoin({ target }: { target: "partner" | "driver" }) {
  ...
  useEffect(() => {
    ...
    let active = true;
    void createClient().rpc("redeem_delivery_partner_invite", { p_token: token }).then(({ data, error }) => {
      if (!active) return;
      if (error) setMessage(error.message);
      else router.replace((data as { role?: string } | null)?.role === "driver" ? "/driver" : "/partner");
    });
    return () => { active = false; };
  }, [auth, router, token]);
