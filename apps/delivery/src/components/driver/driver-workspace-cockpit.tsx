...
function DriverJobCard({ job, updating, onAction }: { job: DriverJob; updating: boolean; onAction: (id: string, action: JobAction, note?: string) => void }) {
  ...
  return <article className="overflow-hidden rounded-2xl border border-[#dce5e0] bg-white shadow-[0_14px_30px_-28px_rgba(30,55,43,0.7)]">
    ...
    <div className="space-y-4 p-4 sm:p-5">
      ...
      {job.status === "assigned" ? <div className="grid grid-cols-2 gap-2"><button type="button" disabled={updating} onClick={() => onAction(job.id, "decline")} className="min-h-12 rounded-xl border border-[#F0C8C2] px-3 py-2.5 text-sm font-semibold text-[#9C2F28] transition hover:bg-[#FFF5F3] disabled:opacity-50">Decline</button><button type="button" disabled={updating} onClick={() => onAction(job.id, "accept")} className="min-h-12 rounded-xl bg-[#F97316] px-3 py-2.5 text-sm font-bold text-[#132C2A] transition hover:bg-[#EA580C] disabled:opacity-50">{updating ? "Accepting…" : "Accept delivery"}</button></div> : null}
      {action && job.status !== "assigned" ? <button type="button" disabled={updating} onClick={() => requestAction(action)} className={`min-h-12 w-full rounded-xl px-3 py-2.5 text-sm font-bold transition disabled:opacity-50 ${action === "delivered" ? "bg-[#16824B] text-white hover:bg-[#12663B]" : "bg-[#F97316] text-[#132C2A] hover:bg-[#EA580C]"}`}>{updating ? "Updating…" : actionLabel}</button> : null}
...
export function DriverWorkspace() {
  ...
  return <main className="min-h-dvh bg-[#FFFDF8] pb-24 text-[#132C2A] sm:pb-10">
