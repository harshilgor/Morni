...
function DriverJobCard({ job, updating, onAction, onRequestHandoff, onVerifyHandoff, onRefresh }: { job: DriverJob; updating: boolean; onAction: (id: string, action: JobAction, note?: string) => void; onRequestHandoff: (id: string, type: "pickup" | "delivery") => Promise<boolean>; onVerifyHandoff: (id: string, type: "pickup" | "delivery", code: string) => Promise<boolean>; onRefresh: () => void }) {
  ...
  return <article className="overflow-hidden rounded-[1.5rem] border border-[#dce5e0] bg-white shadow-[0_14px_30px_-28px_rgba(30,55,43,0.7)]">
    ...
    <div className="space-y-4 p-4 sm:p-5">
      ...
      <div className="grid gap-3 sm:grid-cols-2"><div className={`rounded-xl p-3 ${navigatePickup ? "bg-[#fff5e9] ring-1 ring-[#f6c38e]" : "bg-[#f7faf8]"}`}><p className="text-[10px] font-bold uppercase tracking-[0.13em] text-[#708078]">Pickup store</p><p className="mt-1 text-sm font-semibold leading-5 text-[#33473e]">{job.store_name}</p><p className="mt-1 text-xs leading-5 text-[#60736a]">{job.store_address}</p></div><div className={`rounded-xl p-3 ${!navigatePickup ? "bg-[#edf5fb] ring-1 ring-[#b9d5ec]" : "bg-[#f7faf8]"}`}><p className="text-[10px] font-bold uppercase tracking-[0.13em] text-[#708078]">Customer drop-off</p><p className="mt-1 text-sm font-semibold leading-5 text-[#33473e]">{destination}</p></div></div>
      ...
      {action ? <div className="sticky bottom-[calc(4.75rem+env(safe-area-inset-bottom))] z-10 -mx-4 bg-white/95 px-4 py-2 backdrop-blur sm:static sm:mx-0 sm:bg-transparent sm:px-0 sm:py-0"><button type="button" disabled={updating} onClick={() => onAction(job.id, action)} className="min-h-12 w-full rounded-xl bg-[#F97316] px-3 py-2.5 text-sm font-bold text-[#132C2A] transition hover:bg-[#EA580C] disabled:opacity-50">{updating ? "Updating…" : actionLabel}</button></div> : null}
...
function AvailabilityCard({ driver, activeJobs, updating, online, onAvailability }: { driver: DriverData["driver"]; activeJobs: number; updating: boolean; online: boolean; onAvailability: (availability: DriverAvailability) => void }) {
  ...
  return <section className={`rounded-[1.5rem] border p-4 sm:p-5 ${isAvailable ? "border-[#b9d9c7] bg-[#eff9f1]" : "border-[#dce5e0] bg-white"}`}><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex items-center gap-2"><span className={`h-2.5 w-2.5 rounded-full ${isAvailable ? "bg-[#16824B]" : "bg-[#9AA9A2]"}`} /><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#2D7458]">Driver status</p></div><p className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-[#132C2A]">{isAvailable ? "Available for deliveries" : "Getting you ready"}</p><p className="mt-1 text-xs text-[#5d7268]">{isAvailable ? "Dispatch can send you the next nearby pickup." : "You become available automatically when this portal opens."}</p></div>{isAvailable ? <button type="button" onClick={() => onAvailability("offline")} disabled={updating || isBusy || !online} className="min-h-11 rounded-xl border border-[#B9D9C7] bg-white px-4 py-2.5 text-sm font-semibold text-[#155C4B] transition hover:bg-[#F3F8F4] disabled:opacity-50">{updating ? "Going offline…" : "Go offline"}</button> : <span className="rounded-full bg-[#fff5e9] px-3 py-1.5 text-xs font-bold text-[#8a4b2c]">{updating ? "Connecting…" : "Connecting"}</span>}</div><div className="mt-4 grid gap-2 text-xs text-[#4E6D61] sm:grid-cols-3"><span className="flex items-center gap-2 rounded-lg bg-white/80 px-3 py-2"><PortalIcon name="location" className="h-4 w-4 text-[#155C4B]" />{relativeTime(driver.last_location_at)}</span><span className="flex items-center gap-2 rounded-lg bg-white/80 px-3 py-2"><span className={`h-2 w-2 rounded-full ${online ? "bg-[#16824B]" : "bg-[#C83E3E]"}`} />{online ? "Connection healthy" : "You are offline"}</span><span className="rounded-lg bg-white/80 px-3 py-2">{activeJobs} active job{activeJobs === 1 ? "" : "s"}</span></div>{isBusy ? <p className="mt-3 text-xs font-semibold leading-5 text-[#8a5a42]">Finish the current delivery before going offline.</p> : null}</section>;
}
