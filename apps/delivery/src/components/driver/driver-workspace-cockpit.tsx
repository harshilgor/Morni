...
function DeliveryTimeline({ job, destination }: { job: DriverJob; destination: string }) {
  ...
  return <ol className="relative space-y-4 border-l-2 border-[#dbe7e1] pl-10">
    <li className="relative"><span className={`absolute -left-[2.15rem] top-0.5 grid h-7 w-7 place-items-center rounded-full border-2 border-white text-[11px] font-bold shadow-sm ${pickupComplete ? "bg-[#2f8a64] text-white" : "bg-[#f97316] text-[#132c2a]"}`}>{pickupComplete ? <PortalIcon name="check" className="h-4 w-4" /> : "1"}</span><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#708078]">{pickupComplete ? "Picked up" : "Current step · pickup"}</p><p className="mt-1 text-sm font-bold text-[#213d33]">{job.store_name}</p><p className="mt-0.5 text-xs leading-5 text-[#60736a]">{job.store_address}</p></li>
    <li className="relative"><span className={`absolute -left-[2.15rem] top-0.5 grid h-7 w-7 place-items-center rounded-full border-2 border-white text-[11px] font-bold shadow-sm ${headingToCustomer ? "bg-[#2b6cb0] text-white" : "bg-[#edf2ef] text-[#718079]"}`}>{delivered ? <PortalIcon name="check" className="h-4 w-4" /> : "2"}</span><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#708078]">{headingToCustomer ? "Current step · deliver" : "Next · drop-off"}</p><p className="mt-1 text-sm font-bold text-[#213d33]">Customer delivery</p><p className="mt-0.5 text-xs leading-5 text-[#60736a]">{destination}</p></li>
  </ol>;
}
