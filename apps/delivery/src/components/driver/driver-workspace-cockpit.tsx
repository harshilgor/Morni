...
function DriverJobCard({ job, updating, onAction }: { job: DriverJob; updating: boolean; onAction: (id: string, action: JobAction, note?: string) => void }) {
  ...
  return <article className="overflow-hidden rounded-2xl border border-[#dce5e0] bg-white shadow-[0_14px_30px_-28px_rgba(30,55,43,0.7)]">
    ...
    <div className="space-y-4 p-4 sm:p-5">
      ...
      {job.status === "assigned" ? <div className="sticky bottom-[calc(4.75rem+env(safe-area-inset-bottom))] z-10 -mx-4 bg-white/95 px-4 py-2 backdrop-blur sm:static sm:mx-0 sm:bg-transparent sm:px-0 sm:py-0"><div className="grid grid-cols-2 gap-2"><button type="button" disabled={updating} onClick={() => onAction(job.id, "decline")} className="min-h-12 rounded-xl border border-[#F0C8C2] px-3 py-2.5 text-sm font-semibold text-[#9C2F28] transition hover:bg-[#FFF5F3] disabled:opacity-50">Decline</button><button type="button" disabled={updating} onClick={() => onAction(job.id, "accept")} className="min-h-12 rounded-xl bg-[#F97316] px-3 py-2.5 text-sm font-bold text-[#132C2A] transition hover:bg-[#EA580C] disabled:opacity-50">{updating ? "Accepting…" : "Accept delivery"}</button></div></div> : null}
      {action && job.status !== "assigned" ? <div className="sticky bottom-[calc(4.75rem+env(safe-area-inset-bottom))] z-10 -mx-4 bg-white/95 px-4 py-2 backdrop-blur sm:static sm:mx-0 sm:bg-transparent sm:px-0 sm:py-0"><button type="button" disabled={updating} onClick={() => requestAction(action)} className={`min-h-12 w-full rounded-xl px-3 py-2.5 text-sm font-bold transition disabled:opacity-50 ${action === "delivered" ? "bg-[#16824B] text-white hover:bg-[#12663B]" : "bg-[#F97316] text-[#132C2A] hover:bg-[#EA580C]"}`}>{updating ? "Updating…" : actionLabel}</button></div> : null}
      {confirmOpen ? <div className="space-y-3 rounded-xl border border-[#B9D9C7] bg-[#F0FAF3] p-3"><div><p className="text-sm font-semibold text-[#155C4B]">Confirm this delivery step</p><p className="mt-1 text-xs leading-5 text-[#4E6D61]">Only confirm after you have handed the order to the customer.</p></div><label className="flex items-start gap-2 text-xs leading-5 text-[#39594D]"><input type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} className="mt-1 h-4 w-4 accent-[#16824B]" />I confirm the customer received the order.</label><div className="flex gap-2"><button type="button" onClick={() => setConfirmOpen(false)} className="min-h-10 flex-1 rounded-lg border border-[#B9D9C7] px-3 text-xs font-semibold text-[#47665A]">Cancel</button><button type="button" disabled={!confirmed || updating} onClick={() => { setConfirmOpen(false); onAction(job.id, "delivered", "Customer confirmed receipt."); }} className="min-h-10 flex-1 rounded-lg bg-[#16824B] px-3 text-xs font-bold text-white disabled:opacity-50">Confirm delivered</button></div></div> : null}
...
function AvailabilityCard({ driver, activeJobs, updating, online, onAvailability }: { driver: DriverData["driver"]; activeJobs: number; updating: boolean; online: boolean; onAvailability: (availability: DriverAvailability) => void }) {
  ...
  return <section className="rounded-2xl border border-[#CBE1D3] bg-[#EFF9F1] p-4 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex items-center gap-2"><span className={`h-2.5 w-2.5 rounded-full ${isAvailable ? "bg-[#16824B]" : "bg-[#9AA9A2]"}`} /><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#2D7458]">Your availability</p></div><p className="mt-2 text-2xl font-semibold capitalize tracking-[-0.04em] text-[#132C2A]">{driver.availability}</p></div>{isAvailable ? <button type="button" onClick={() => onAvailability("offline")} disabled={updating || isBusy} className="min-h-11 rounded-xl border border-[#B9D9C7] bg-white px-4 py-2.5 text-sm font-semibold text-[#155C4B] transition hover:bg-[#F3F8F4] disabled:opacity-50">Go offline</button> : <button type="button" onClick={() => onAvailability("available")} disabled={updating || !online} className="min-h-11 rounded-xl bg-[#F97316] px-4 py-2.5 text-sm font-bold text-[#132C2A] transition hover:bg-[#EA580C] disabled:opacity-50">{updating ? "Going available…" : "Go available"}</button>}</div><div className="mt-4 grid gap-2 text-xs text-[#4E6D61] sm:grid-cols-2"><span className="flex items-center gap-2 rounded-lg bg-white/80 px-3 py-2"><PortalIcon name="location" className="h-4 w-4 text-[#155C4B]" />{relativeTime(driver.last_location_at)}</span><span className="flex items-center gap-2 rounded-lg bg-white/80 px-3 py-2"><span className={`h-2 w-2 rounded-full ${online ? "bg-[#16824B]" : "bg-[#C83E3E]"}`} />{online ? "Connection is healthy" : "You are offline"}</span></div><p className="mt-3 text-xs leading-5 text-[#4E6D61]">When you are available, Morni uses your location to prioritize nearby pickup requests.</p></section>;
}
...
export function DriverWorkspace() {
  ...
  return <main className="min-h-dvh bg-[#FFFDF8] pb-[calc(7rem+env(safe-area-inset-bottom))] text-[#132C2A] sm:pb-10">
