...
function DeliveryTimeline({ job, destination }: { job: DriverJob; destination: string }) {
  ...
  return <ol className="relative space-y-5">
    <li className="relative grid min-w-0 grid-cols-[2rem_minmax(0,1fr)] gap-3"><span className={`relative z-10 grid h-7 w-7 place-items-center self-start rounded-full border-2 border-white text-[11px] font-bold shadow-sm ${pickupComplete ? "bg-[#2f8a64] text-white" : "bg-[#f97316] text-[#132c2a]"}`}>{pickupComplete ? <PortalIcon name="check" className="h-4 w-4" /> : "1"}</span><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#708078]">{pickupComplete ? "Picked up" : "Current step · pickup"}</p><p className="mt-1 break-words text-sm font-bold text-[#213d33]">{job.store_name}</p><p className="mt-0.5 break-words text-xs leading-5 text-[#60736a]">{job.store_address}</p></div></li>
    <li className="relative grid min-w-0 grid-cols-[2rem_minmax(0,1fr)] gap-3"><span className={`relative z-10 grid h-7 w-7 place-items-center self-start rounded-full border-2 border-white text-[11px] font-bold shadow-sm ${headingToCustomer ? "bg-[#2b6cb0] text-white" : "bg-[#edf2ef] text-[#718079]"}`}>{delivered ? <PortalIcon name="check" className="h-4 w-4" /> : "2"}</span><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#708078]">{headingToCustomer ? "Current step · deliver" : "Next · drop-off"}</p><p className="mt-1 break-words text-sm font-bold text-[#213d33]">Customer delivery</p><p className="mt-0.5 break-words text-xs leading-5 text-[#60736a]">{destination}</p></div></li>
    <span aria-hidden="true" className="pointer-events-none absolute left-[0.9rem] top-7 bottom-7 w-0.5 bg-[#dbe7e1]" />
  </ol>;
}

function AvailabilityCard(_: { driver: DriverData["driver"]; activeJobs: number; updating: boolean; online: boolean; onAvailability: (availability: DriverAvailability) => void }) {
  return null;
}

function DriverJobCard({ job, updating, canAccept = true, onAction, onRequestHandoff, onVerifyHandoff, onRefresh }: { job: DriverJob; updating: boolean; canAccept?: boolean; onAction: (id: string, action: JobAction, note?: string) => void; onRequestHandoff: (id: string, type: "pickup" | "delivery") => Promise<boolean>; onVerifyHandoff: (id: string, type: "pickup" | "delivery", code: string) => Promise<boolean>; onRefresh: () => void }) {
  ...
  const navigatePickup = ["unassigned", "assigned", "accepted", "at_pickup"].includes(job.status);
  ...
  const action: JobAction | null = ["unassigned", "assigned"].includes(job.status) ? "accept" : job.status === "accepted" ? "at_pickup" : null;
  const actionLabel = ["unassigned", "assigned"].includes(job.status) ? "Accept delivery" : "I am at the store";
  ...
  async function captureProof(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      onRefresh();
      // The store has already generated the pickup code. Open the entry field
      // as soon as the parcel photo succeeds so the handoff is one continuous
      // action for the rider.
      if (job.status === "at_pickup") setHandoffType("pickup");
      if (job.status === "collected") {
        if (job.delivery_handoff_status === "pending") setHandoffType("delivery");
        else void startHandoff("delivery");
      }
  ...
  return <article className="driver-job-card overflow-hidden rounded-[1.5rem] border border-[#dce5e0] bg-white shadow-[0_14px_30px_-28px_rgba(30,55,43,0.7)]">
    ...
    <div className="space-y-4 p-4 sm:p-5">
      ...
      {job.item_image_urls?.length ? <div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#708078]">Order preview</p><div className="mt-2 flex gap-2 overflow-x-auto pb-1">{job.item_image_urls.slice(0, 4).map((url, index) => <div key={`${url}-${index}`} className="relative h-20 w-20 shrink-0 overflow-hidden rounded-xl border border-[#dce5e0] bg-[#f3f7f4]"><Image src={url} alt={`Ordered item ${index + 1}`} fill sizes="80px" className="object-cover" /></div>)}</div></div> : null}
      ...
      {job.status === "at_pickup" ? <div className="rounded-xl border border-[#f1c58e] bg-[#fff7ed] px-3 py-3"><p className="text-sm font-bold text-[#8a4b2c]">Collect parcel: photo + OTP</p><p className="mt-1 text-xs leading-5 text-[#8a5a42]">Photograph the parcel, then enter the 6-digit code shown in the store owner’s portal.</p><label className="mt-3 flex min-h-11 cursor-pointer items-center justify-center gap-2 rounded-lg bg-white px-3 py-2 text-xs font-bold text-[#8a4b2c] ring-1 ring-[#f1c58e] hover:bg-[#fffaf4]"><PortalIcon name="camera" className="h-4 w-4" />{proofLoading ? "Uploading photo…" : proofCount > 0 ? `${proofCount} parcel photo${proofCount === 1 ? "" : "s"} captured` : "Take parcel photo"}<input type="file" accept="image/jpeg,image/png,image/webp" capture="environment" className="sr-only" onChange={(event) => void captureProof(event)} disabled={proofLoading} /></label>{proofError ? <p className="mt-2 text-xs text-[#a53b30]">{proofError}</p> : null}{proofCount > 0 ? <button type="button" onClick={() => setHandoffType("pickup")} disabled={handoffLoading} className="mt-3 min-h-10 w-full rounded-lg bg-[#F97316] px-3 py-2 text-xs font-bold text-[#132C2A] disabled:opacity-50">Enter pickup OTP</button> : <p className="mt-2 text-xs font-semibold text-[#a65316]">A parcel photo is required before OTP verification.</p>}</div> : null}
      ...
      <div className="flex flex-wrap items-center gap-2"><a href={mapsHref} target="_blank" rel="noreferrer" className="flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-[#155C4B] px-3 py-2.5 text-sm font-semibold text-white transition hover:bg-[#0F4639]"><PortalIcon name="location" className="h-4 w-4" />{navigatePickup ? "Navigate to store" : "Navigate to customer"}</a><button type="button" onClick={() => void copyAddress()} className="grid min-h-11 min-w-11 place-items-center rounded-xl border border-[#D3DDD8] px-3 text-xs font-semibold text-[#155C4B] transition hover:bg-[#F3F8F4]" aria-label="Copy current address">{copied ? "Copied" : "Copy"}</button>{job.delivery_phone ? <><a href={`tel:${job.delivery_phone}`} className="grid min-h-11 min-w-11 place-items-center rounded-xl border border-[#D3DDD8] text-[#155C4B] transition hover:bg-[#F3F8F4]" aria-label="Call customer"><PortalIcon name="phone" className="h-4 w-4" /></a><a href={`tel:${job.delivery_phone}`} className="w-full text-xs font-semibold text-[#356a5a] underline-offset-2 hover:underline sm:w-auto">Customer: {job.delivery_phone}</a></> : null}</div>
 {action ? <div className="sticky bottom-[calc(4.75rem+env(safe-area-inset-bottom))] z-10 -mx-4 rounded-t-2xl border-t border-[#e6eeea] bg-white/95 px-4 py-3 shadow-[0_-8px_24px_rgba(19,44,42,0.08)] backdrop-blur sm:static sm:mx-0 sm:rounded-xl sm:border sm:bg-[#fbfdfc] sm:px-4 sm:shadow-none"><button type="button" disabled={updating || (action === "accept" && !canAccept)} onClick={() => onAction(job.id, action)} className="min-h-12 w-full rounded-xl bg-[#F97316] px-3 py-2.5 text-sm font-bold text-[#132C2A] transition hover:bg-[#EA580C] disabled:opacity-50">{updating ? "Updating…" : action === "accept" && !canAccept ? "Finish current delivery first" : actionLabel}</button></div> : null}
...
function AssignmentNotice({ job, alertsEnabled, onEnableAlerts, onDismiss }: { job: DriverJob; alertsEnabled: boolean; onEnableAlerts: () => void; onDismiss: () => void }) {
  return <section className="mx-auto mt-4 w-[min(100%-2rem,72rem)] rounded-2xl border border-[#F3C48D] bg-[#FFF4E8] p-4 shadow-[0_16px_40px_-28px_rgba(185,89,22,0.5)] sm:flex sm:items-center sm:justify-between sm:gap-5"><div className="flex gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#F97316] text-[#132C2A]"><PortalIcon name="bell" className="h-5 w-5" /></span><div><p className="text-sm font-bold text-[#8A3E08]">New delivery offer</p><p className="mt-1 text-xs leading-5 text-[#7A512F]">{job.order_number} from {job.store_name}. You and other riders can accept it—the first acceptance wins.</p></div></div><div className="mt-3 flex gap-2 sm:mt-0">{!alertsEnabled ? <button type="button" onClick={onEnableAlerts} className="min-h-10 rounded-lg border border-[#E6B879] bg-white px-3 text-xs font-semibold text-[#8A3E08]">Enable alerts</button> : null}<button type="button" onClick={onDismiss} className="min-h-10 rounded-lg px-3 text-xs font-semibold text-[#7A512F]">Dismiss</button></div></section>;
}
...
export function DriverWorkspace() {
  ...
  useEffect(() => {
    const assigned = data?.jobs.filter((job) => job.status === "assigned" || job.status === "unassigned") ?? [];
  ...
  const otherJobs = currentJob
    ? [...activeJobs.filter((job) => job.id !== currentJob.id), ...data!.jobs.filter((job) => job.status === "unassigned")]
    : [];
  ...
  // Keep a generous scroll clearance below the fixed mobile nav. The nav can
  // grow with larger text/accessibility settings, and Android gesture insets
  // add space beyond its visual height.
  return <main className="driver-app min-h-dvh pb-[calc(12rem+env(safe-area-inset-bottom))] sm:pb-10">
    <header className="sticky top-0 z-20 border-b border-[#dce5e0] bg-white/95 backdrop-blur"><div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3 sm:px-6"><div className="flex min-w-0 items-center gap-3"><BrandMark className="h-10 w-10 object-contain" /><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#6b8077]">Morni rider</p><p className="truncate text-lg font-semibold">Hello, {driver.display_name.split(" ")[0]}</p></div></div><div className="flex items-center gap-2"><button type="button" disabled={updating === "availability" || !online} onClick={() => void setAvailability(driver.availability === "available" ? "paused" : "available")} className={`flex h-10 items-center gap-2 rounded-xl px-3 text-xs font-bold transition disabled:opacity-50 ${driver.availability === "available" ? "bg-[#e8f4ee] text-[#21634f] hover:bg-[#dff0e6]" : "bg-[#f1f4f2] text-[#52675d] hover:bg-[#e7ece9]"}`} aria-label={driver.availability === "available" ? "Pause delivery requests" : "Go online to receive delivery requests"}><span className={`h-2 w-2 rounded-full ${driver.availability === "available" ? "bg-[#2f8a64] shadow-[0_0_0_4px_rgba(47,138,100,0.12)]" : "bg-[#9aa9a2]"}`} /><span>{updating === "availability" ? "Updating…" : driver.availability === "available" ? "Online" : driver.availability === "paused" ? "Paused" : "Offline"}</span></button><span className="hidden text-[11px] text-[#819089] lg:inline">{lastUpdatedAt ? `Updated ${formatTime(lastUpdatedAt)}` : "Connecting"}</span><button type="button" onClick={() => void load(true)} disabled={refreshing || !online} className="grid h-10 w-10 place-items-center rounded-xl border border-[#d7e1dc] bg-white text-[#4d6c60] transition hover:bg-[#f5f8f6] disabled:opacity-50" aria-label="Refresh deliveries"><PortalIcon name="refresh" className={`h-4 w-4 ${refreshing ? "animate-spin" : ""}`} /></button></div></div></header>
